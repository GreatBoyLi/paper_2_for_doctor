...
TARGET_METADATA_FILE = PROJECT_DIR / settings["paths"]["target_metadata"]
CHECKPOINT_DIR = PROJECT_DIR / settings["paths"]["checkpoint_dir"]
TRAINING_HISTORY_DIR = PROJECT_DIR / settings["paths"]["training_history_dir"]
EVALUATION_DIR = PROJECT_DIR / settings["paths"]["evaluation_dir"]
TARGET_TIMEZONE = settings["target"]["timezone"]
TARGET_TEST_PERIODS = settings["target"]["test_periods"]
HORIZON_STEPS = settings["evaluation"]["horizon_steps"]
...
TRAINING_EPOCHS = settings["training"]["epochs"]
EARLY_STOPPING_PATIENCE = settings["training"]["early_stopping_patience"]
EARLY_STOPPING_MIN_DELTA = settings["training"]["early_stopping_min_delta"]
DOMAIN_HIDDEN_DIM = settings["domain_adversarial"]["hidden_dim"]
GRL_LAMBDA = settings["domain_adversarial"]["grl_lambda"]
