...
class DataPipelineEntrypointsTests(unittest.TestCase):
    ...
    def test_target_pipeline_has_the_five_required_steps_in_order(self):
        self.assertEqual(
            [step.script.name for step in TARGET_STEPS],
            [
                "prepare_target_data.py", "extract_target_period.py", "prepare_target_dataset.py",
                "prepare_target_test_dataset.py", "build_target_graph.py",
            ],
        )
